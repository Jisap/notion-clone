...
const Item = ({ 
   id,
   label, 
   onClick,    // función de redirect
   icon:Icon,  
   active,
   documentIcon,
   isSearch,
   level = 0,
   onExpand,   // actualiza el state
   expanded,   // state
}: ItemProps) => {
   ...
   return (
      ...
      >
         {!!id && ( // Si existe el documento se muestra el icon ChevronRight
         ...
         {documentIcon ? ( // Si el documentIcon se provee se usa
            ...
            {isSearch && (  // Si se provee isSearch se muestra Ctrl + k despues de icon y el label
